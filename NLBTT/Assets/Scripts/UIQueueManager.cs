...
public class UIQueueManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Queues a complex event to be shown (alias for QueueEventUI)
    /// </summary>
    public void QueueComplexEvent(ComplexEventCard eventCard)
    {
        if (eventCard == null)
        {
            Debug.LogError("UIQueueManager: Cannot queue null complex event card!");
            return;
        }

        LogDebug($"Queueing complex event: {eventCard.GetEventTitle()}");
        QueueEventUI(eventCard);
    }
